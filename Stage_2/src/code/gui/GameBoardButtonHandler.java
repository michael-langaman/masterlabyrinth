...
/**
 * This is the Handler for the 49 JButtons in the GameBoard, corresponding to the tiles in the game.  It
 * allows tiles to be clicked on in the GUI.  When clicked, the free tile will try to be inserted at the tile location
 * clicked on.
...
public class GameBoardButtonHandler implements ActionListener{
	...
	/**
	 * refers to the rows of the gameboard that the handler will be associated with
	 */
	private int _row;
	
	/**
	 * refers to the columns of the gameboard that the handler will be associated with
	 */
	private int _col;
	
	/**
	 * refers to the gameboard object that the handler will be associated with
	 */
	GameBoard _gb;
	
	/**
	 * refers to the gameboardGUI object that the handler will be associated with
	 */
	GameBoardGUI _gbGUI;
	
	/**
	 * This is an explicit constructor for the handler, and it sets the row and column position of the handler which
	 * corresponds to a position on the gameboard's board.  It also sets what gameboard the button is on, and the associated
	 * gameboardGUI.
	 * 
	 * @param i the row that the button is placed on
	 * @param j the column that the button is placed on
	 * @param gb gameboard object that the button is placed on
	 * @param gbGUI GameBoardGUI object associated with the gameboard
	 * @author Ian,Ken 04-10-16
	 */
	 ...
	 * If the player hasn't moved and hasn't inserted a tile yet, then the free tile will be inserted at the position clicked
	 * provided it's a valid position.
	 * 
