 ...
 * @author Tyler Barrett, William Stewart, Michael Langman, Fen Qin, Daniel Palacio
 * 
...
public class GameBoard {
	 ...
	 * on the gameboard. Once the board is populated, this list holds only the single shiftable tile not
	 * on the gameboard.
	 */
...
//	private int _turns;
	...
	/**
	 * Constructor for a GameBoard which builds up the game from a SaveFile state,
	 * rather than randomizing the initial state of the game.
	 * 
	 * @param saveFile the SaveFile instance representing the state of the game to be loaded
	 */
	public GameBoard(SaveFile saveFile) {
		this(saveFile.getPlayerNum());
		ArrayList<MoveableTile> t= new ArrayList<MoveableTile>();
		ArrayList<Player> p = new ArrayList<Player>();
		
		p = saveFile.getPlayers();
		t = saveFile.getTiles();
		
		System.out.println("We're in the gameboard loading constructor! and # tiles is:  " + t.size());
		
		for (int i = 0; i < _numOfPlayers; i++) {
			_players[i] = p.get(i);
			_players[i].setGameBoard(this);
		}		
		for (int i = 0; i<7; i++){
			for (int j = 0; j<7 ; j++){
		...
		_arrayOfMoveTiles.add(t.get(0));
		CURRENTPLAYER = _players[0]; 
		_tokens = saveFile.getTokens();
		_currentTargetToken = _tokens.get(saveFile.getMaxToken());
	}
	...
	/**
	 * This method adds tokens to arraylist
	...
	/**
	 * Returns the Token instance for the next token to be picked up.
	...
	/**
	 * Returns the number for the next token to be picked up.
	...
	/**
	 * This method instantiates 16 tiles of the FixedTile class that are then
	...
	/**
	 * This method sets this GameBoard's list of MoveableTiles to the specified ArrayList.
	 * 
	 * @param aL is the ArrayList of moveable tiles. The location of these particular tiles are the same each time the board is created.
	 * 
	...
	/** 
	 * The method is a getter for the board which returns the board (a 2D-array of AbstractTiles).
	 * 
	 ...
	 * used when randomizing tile orientations.
	 * 
	...
	/**
	 * This method puts the movable tiles onto the 2D board array.
	 ...
	 * by two, only reaching the places where there isn't a fixed tile, at indices 1, 3, or 5.
	 * Then after placing the tile it removes it from the arraylist
	 * so that the next tile is the next item from the arraylist.
	 * Then once it's done for the row it goes to the next column 
	...
	/**This method populates the board with between 1 and 4 players using the color order maintained in the Player class, at the 
	...
	*/
	...
	/**
	 * The method is a getter that returns the tile at the specified coordinates
	...
	/**
	 * Sets the tile at the specified coordinates in the 2D gameboard array.
	 * @param row the row index of the tile's desired position
	 * @param column the column index of the tile's desired position
	 * @param tile the tile to be set in the array
	 */
	public void setTileAtCoordinates (int row, int column, AbstractTile tile) {
	 ...
	 * reference to tile object in _board.  Returns a tile if tileNum in range [0,48] and
	 * null otherwise.
	...
	/**
	 * This method updates the game feedback panel in the GUI.
...
//			System.out.println("Invalid Move: There exists NO path between origin and destination.");
	...
	/**
	 * The method is a getter which returns the board's array of players.
...
//	AND MAY BE USEFUL LATER, SO WE LEAVE IT IN.
	...
	/**
	 * This methods retrieves the gameboard's reference to its GUI.
	 * @return the GUI for this gameboard
	 * @author Ian,Weijin 
	 */
...
//	}
	...
	/**
	 * If the proper conditions are met (i.e. it is the beginning of a player's turn), the state
	 * of the game is saved to a file as 3 lines of text: a list of player records, a list of
	 * tile records, and an index representing the last shift performed.
	 * @author Will, Tyler
	 * @param s a path to the desired file location
	 * @return a boolean which represents whether or not the save operation was successful
	 */
	public boolean save(String s) {
		if (!CURRENTPLAYER.getHasMovedThisTurn() && !CURRENTPLAYER.getHasInsertedThisTurn()) {
			try (PrintWriter stream = new PrintWriter( new File(s+".mls") )) {
				stream.println(printPlayerRecords());
				stream.println(printBoardState());
				stream.print(printLastShift());
	...
	/**
	 * A private helper method for save() which returns a String representing the records
	 * for each player currently in the game. These are printed as a comma-separated list
	 * of entries: [name,color,wand count,[comma-separated list of token numbers on formula card],[comma-separated list of token numbers collected]].
	 * @author Will, Tyler
	 * @return a String for the save file representing the state of each player
	 */
	public String printPlayerRecords() {
		String s = "";
		int i = 0;
		for (i = 0; i < _numOfPlayers; i++) {
			s = s + "[";
			s = s + _players[(i+currentPlayerIndex)%_numOfPlayers].getName() + ",";
			s = s + _players[(i+currentPlayerIndex)%_numOfPlayers].getColor() + ",";
			s = s + _players[(i+currentPlayerIndex)%_numOfPlayers].getWands() + ",[";
			for (Token formulaCard : _players[(i+currentPlayerIndex)%_numOfPlayers].getFormulaCard().getCardTokens()){
				s = s + formulaCard.getValue() + ",";
			}
			System.out.println(s);
			s = s.substring(0,s.length()-1) + "],[";
			System.out.println(s);
			if (!_players[(i+currentPlayerIndex)%_numOfPlayers].getTokens().isEmpty()){
				for (Token myToken : _players[(i+currentPlayerIndex)%_numOfPlayers].getTokens()){
					s = s + myToken.getValue() + ",";
				}
				s = s.substring(0,s.length()-1) + "]";
				s = s + "],";	
			}
	...
	/**
	 * A private helper method for save() which returns a String representing
	 * all tiles on the board (printed in order from left-to-right, top-to-bottom),
	 * using the AbstractTile's toString() method.
	 * @author Will, Tyler
	 * @return a String for the save file representing the state of the game board
	 */
	public String printBoardState() {
		String result = "";
		for (int row = 0; row < 7; row++) {
			for (int column = 0; column < 7; column++) {
				result += _board[row][column].toString();
				if (row < 6 || column < 6) result += ",";
			}
		}
		return result;
	}
	...
	/**
	 * A private helper method for save() which returns a String representing
	 * the index last used for shifting the board. The save file format's convention
	 * for labeling these indices counts from 1 in a clockwise fashion (starting from
	 * the top-left), but the code base's implementation labels this data according
	 * to the index of the board position where the free tile was inserted, in range [0,48].
	 * Hence, a mapping must be done from one convention to the other.
	 * @author Will, Tyler
	 * @return a String for the save file representing the last shift on the gameboard
	 */
	public String printLastShift() {
		int lastShiftPosition = _arrayOfMoveTiles.get(0).getLastTileNum();
		switch (lastShiftPosition) {
		case 1: return "1";
		case 3: return "2";
		case 5: return "3";
		case 13: return "4";
