...
public abstract class AbstractTile {
	...
	/**
	 * holds the string containing the shape of the tile, I, T, or L
	 */
	private String _identity;
	
	/**
	 * Holds the integer rotation of the tile as defined in the stage 3 requirements documents.
	 * 
	 * Since it depends on the shape, here are the rotations:
	 * T:
	 * 		0:  up - off, down - on, left - on, right - on
	 * 		1:  up - on, down - on, left - on, right - off 
	 * 		2:  up - on, down - off, left - on, right - on
	 * 		3:  up - on, down - on, left - off, right - on
	 * L:
	 * 		0:  up - off, down - on, left - off, right - on
	 * 		1:  up - off, down - on, left - on, right - off
	 * 		2:  up - on, down - off, left - on, right - off
	 * 		3:  up - on, down - off, left - off, right - on
	 * I:
	 * 		0 and 2:  up - on, down - on, left - off, right - off
	 * 		1 and 3:  up - off, down - off, left - on, right - on
	 */
	...
	/**
	 * Explicit constructor for an abstract tile, that sets the identity, rotation, and open paths
	 * for the tile.  This is used for restoring the save file.
	 * @param identity - T, L, or I -- string representing the shape of the tile
	 * @param top - the value (1 or 0) of the top path of the tile
	 * @param bottom - the value (1 or 0) of the bottom path of the tile
	 * @param left - the value (1 or 0) of the left path of the tile
	 * @param right - the value (1 or 0) of the right path of the tile
	 * @param rotation - rotation value of the tile (see the documentation for the _rotation instance variable for more information)
	 */
	public AbstractTile (String identity, int top, int bottom, int left, int right, int rotation) {
		_players = new ArrayList<Player>();
		_identity = identity;
		_rotation = rotation;
		setDirections(top, bottom, left, right);
	}
	...
	public boolean setDirections(int t, int b, int l, int r){
		if(checkLegalDirectionsInput(t,b,l,r)){	
			_top = t;
			_bottom = b; 
			_left = l;
			_right = r;
	...
	/**
	 * Returns a string representing the shape of the tile, I, T, or L.
	 * @return string containing the identity of the tile (ie, its shape, I, T, L)
	 */
	public String getIdentity() {
		return _identity;
	}
	/**
	 * Set the identity (shape) of the tile.  Used for loading save files.
	 * @param id - The shape of the tile, should be read from the save file.
	 */
	...
	/**
	 * Returns a String representing the state of this tile, printed in the following format:
	 * [type,token value,[comma-separated list of players by color]], where "type" contains
	 * a char representing the paths on the tile (T for T-shaped, L for L-shaped, and I for
	 * a straight-across path) and an integer representing the rotation of the tile from a
	 * default "0" rotation (this convention is explained at the _rotation instance variable),
	 * e.g. printed as T0 or L2.
	 * @author Tyler, Will
	 * @return a String for the save file representing the state of this tile
	 */
	public String toString() {
		String result = "[";
		result = result + _identity + _rotation + ",";
		result = result + (this.hasToken() ? _token.getValue() : 0) + ",[";
		for (int i = 0; i < _players.size(); i++) {
			result += _players.get(i).getColor();
			if (i < _players.size() - 1) result += ",";
		}
		result += "]]";
		return result;
	}
